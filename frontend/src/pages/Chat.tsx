...
export default function Chat() {
  ...
  // Buscar instâncias distintas da tabela webhook_messages
  useEffect(() => {
    async function fetchInstances() {
      ...
      const { data, error } = await supabase
        ...
        .select('instance');
      if (!error && data) {
        const uniqueInstances = Array.from(new Set(data.map((msg: any) => msg.instance)))
          .filter((v) => v && v !== '')
          .map((v) => ({ value: v }));
        setInstances(uniqueInstances);
        if (uniqueInstances.length > 0) setSelectedInstance(uniqueInstances[0].value);
      }
      setLoading(false);
    }
    fetchInstances();
      ...
      if (!error && data) {
        ...
        data.forEach((msg: any) => {
          if (!uniqueMap.has(msg.remote_jid)) {
            uniqueMap.set(msg.remote_jid, { remote_jid: msg.remote_jid, id: msg.remote_jid, push_name: msg.push_name });
          }
        });
        const uniqueContacts = Array.from(uniqueMap.values())
          .filter((c) => c.remote_jid && c.remote_jid !== '');
        setContacts(uniqueContacts);
        if (uniqueContacts.length > 0) setSelectedContact(uniqueContacts[0].remote_jid);
      } else {
        setContacts([]);
        setSelectedContact('');
      }
  ...
  useEffect(() => {
    ...
    fetchMessages();
  }, [selectedContact, selectedInstance]);
  ...
  const handleSend = (e: React.FormEvent) => {
    ...
    setInput('');
    // Aqui você pode integrar o envio real para Evolution
  };
    ...
    <div className="chat-whatsapp-root">
      <aside className="chat-whatsapp-sidebar">
        ...
        <div className="chat-whatsapp-contacts">
          {loadingContacts ? <Loader2 className="animate-spin" /> : (
            <ul>
              {contacts.map(contact => (
                <li
                  key={contact.remote_jid}
                  className={selectedContact === contact.remote_jid ? 'selected' : ''}
                  onClick={() => handleContactChange(contact.remote_jid)}
                >
                  <span className="contact-avatar">{(contact.push_name && !contact.remote_jid.endsWith('@g.us')) ? contact.push_name[0] : contact.remote_jid[0]}</span>
                  <span className="contact-name">{(contact.push_name && !contact.remote_jid.endsWith('@g.us')) ? contact.push_name : contact.remote_jid}</span>
                </li>
              ))}
            </ul>
          )}
      ...
      <main className="chat-whatsapp-main">
        ...
        <section className="chat-whatsapp-messages">
          {loadingMessages ? <Loader2 className="animate-spin" /> : (
            messages.length === 0 ? (
              <div style={{ color: '#b8c1ec', textAlign: 'center', marginTop: 40 }}>Nenhuma mensagem</div>
            ) : (
              messages.map((msg, idx) => (
                <div key={msg.id || idx} className={`chat-message ${msg.from_me ? 'me' : 'them'}`}>
                  <div className="chat-message-bubble">
                    {msg.message_content}
                    <span className="chat-message-time">{new Date(msg.message_timestamp).toLocaleString()}</span>
                  </div>
                </div>
              ))
            )
        ...
        <form className="chat-whatsapp-input-bar" onSubmit={handleSend}>
          ...
          />
          <button type="submit" disabled={!input.trim() || !selectedContact}>Enviar</button>
        </form>
      </main>
    </div>
  );
...
// Crie o arquivo src/pages/Chat.css com o visual moderno sugerido 